...
public:
  ...
  void list_intr_2(std::vector<int> id1,std::vector<int> id2,double d12l,double d12h,std::vector<int> &idx1,std::vector<int> &idx2) {
    long ctr=0;
    for(auto i=0;i<id1.size();++i) {
      for(auto j=0;j<id2.size();++j) {
        if(interact_2(id1[i],id2[j],d12l,d12h)) {
          idx1.push_back(id1[i]);
          idx2.push_back(id2[j]);
        }
      }
    }
  }
...
public:
  ...
  void tcf_2_all_dt_im(const string atm1,const string atm2,double d12l,double d12h,std::vector<array<double,2>> &v) {
    array<double,2> ar;
    std::vector<int> idx1;
    std::vector<int> intx1;
    std::vector<int> intx2;
    std::vector<int> idx2;
    double tcf,tcfx;
    framx[0].get_idx_of(atm1,idx1);
    framx[0].get_idx_of(atm2,idx2);
    vector<std::vector<int>> ir1;
    vector<std::vector<int>> ir2;
    for(auto i=0;i<framx.size();++i){
      intx1.clear();
      intx2.clear();
      framx[i].list_intr_2(idx1,idx2,d12l,d12h,intx1,intx2);
      ir1.push_back(intx1);
      ir2.push_back(intx2);
    }
    for(int dt=0;dt<framx.size();++dt) {
      ar[0]=dt;
      tcf=0;
      for(int d=0;d<framx.size()-dt;++d) {
        tcfx=0;
        for(int i=0;i<ir1[d].size();++i) {
          if(framx[dt+d].interact_2(ir1[d][i],ir2[d][i],d12l,d12h)) {
              tcfx+=1;
          }
        }
        tcf+=((tcfx/ir1[d].size())/(framx.size()-dt));
      }
      ar[1]=tcf;
      v.push_back(ar);
    }
  ...
  void tcf_2_all_dt(const string atm1,const string atm2,double d12l,double d12h,std::vector<array<double,2>> &v) {
    array<double,2> ar;
    std::vector<int> idx1;
    std::vector<int> intx1;
    std::vector<int> intx2;
    std::vector<int> idx2;
    double tcf,tcfx,tcfy;
    framx[0].get_idx_of(atm1,idx1);
    framx[0].get_idx_of(atm2,idx2);
    vector<std::vector<int>> ir1;
    vector<std::vector<int>> ir2;
    ...
    for(int dt=0;dt<framx.size();++dt) {
	  ...
	  tcfy=1;
	  for(int j=0;j<=dt;++j) {
              if(!(framx[j+d].interact_2(ir1[d][i],ir2[d][i],d12l,d12h))) {
                  tcfy=0;
		  break;
	      }
          }
